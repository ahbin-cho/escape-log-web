...
function Scale({
  ...
  return (
    <div>
      <label className="mb-1.5 block text-sm font-bold text-cream/50">{label}</label>
...
const field =
  "w-full border-b-2 border-edge/30 bg-transparent px-1 py-2.5 text-sm outline-none placeholder:text-cream/25 focus:border-candy";
    ...
    <form onSubmit={handleSubmit} className="space-y-5">
      <div className="grid gap-4 sm:grid-cols-2">
        <div className="sm:col-span-2">
          <label className="mb-1.5 block text-sm font-bold text-cream/50">테마명 *</label>
          <input
            className={field}
        ...
        <div>
          <label className="mb-1.5 block text-sm font-bold text-cream/50">매장명</label>
          <input
            className={field}
            value={form.cafeName}
        ...
        <div>
          <label className="mb-1.5 block text-sm font-bold text-cream/50">플레이 날짜</label>
          <input
            type="date"
            className={field}
            value={form.playedAt}
        ...
        <div>
          <label className="mb-1.5 block text-sm font-bold text-cream/50">장르</label>
          <select
            className={field}
            value={form.genre}
        ...
        <div className="flex items-end gap-3">
          <label className="flex cursor-pointer items-center gap-2 text-sm font-bold text-cream/50">
            <input
              type="checkbox"
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        <div>
          <label className="mb-1.5 block text-sm font-bold text-cream/50">남은 시간</label>
          <input
            className={field}
        ...
        <div>
          <label className="mb-1.5 block text-sm font-bold text-cream/50">사용 힌트 수</label>
          <input
            type="number"
            min="0"
            className={field}
      ...
      <div>
        <label className="mb-1.5 block text-sm font-bold text-cream/50">
          한줄 후기 <span className="font-normal text-cream/25">(스포일러 없이)</span>
        </label>
        <input
          className={field}
          value={form.oneLiner}
      ...
      <div>
        <label className="mb-1.5 block text-sm font-bold text-cream/50">
          메모 <span className="font-normal text-cream/25">(비공개)</span>
        </label>
        <textarea
          rows={4}
          className={field}
      ...
      <div className="flex flex-col-reverse gap-3 sm:flex-row sm:justify-between">
        ...
        <div className="flex gap-3">
          <button
            type="button"
            onClick={() => router.push("/")}
            className="flex-1 rounded-xl border-2 border-edge/40 px-4 py-2 text-sm font-bold text-cream/50 transition active:scale-[0.97] hover:border-edge sm:flex-none"
          >
            취소
          </button>
          <button
            type="submit"
            className="rough flex-1 rounded-xl border-2 border-edge bg-candy px-6 py-2 text-sm font-bold text-white shadow-cute transition active:scale-[0.97] sm:flex-none"
          >
            저장
          </button>
        </div>
